...
export const createOccasionField = async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: `${data.length} Occasion field(s) created successfully`, data });
  } catch (error) {
    if (error.name === "SequelizeUniqueConstraintError") {
      return res.status(409).json({ success: false, message: "Duplicate entry", details: error.errors });
    }
    if (error.name === "SequelizeForeignKeyConstraintError") {
      return res.status(400).json({ success: false, message: "Invalid occasion_id" });
    }
...
export const updateOccasionField = async (req, res) => {
  try {
    const { id } = req.params;
    const [updated] = await updateOccasionFieldService(id, req.body, req.admin?.id);

    if (!updated) return res.status(404).json({ success: false, message: "Occasion field not found or no changes made" });

    res.status(200).json({ success: true, message: `Occasion field with ID (${id}) updated successfully` });
  } catch (error) {
    handleSequelizeError(error, res);
  }
};
...
export const deleteOccasionField = async (req, res) => {
  try {
    ...
    if (!deleted) return res.status(404).json({ message: "Occasion field not found" });
    res.status(200).json({ message: `Occasion field with ID (${id}) deleted successfully` });
  } catch (error) {
    handleSequelizeError(error, res);
  }
};
...
export const getOccasionFieldsById = async (req, res) => {
  try {
    ...
    if (!data) return res.status(404).json({ success: false, message: "No occasion fields found" });
