...
import { findAdminByEmail, findAdminById, saveAdmin } from "../repositories/authRepository.js";
...
export const loginService = async ({ email, password }) => {
  ...
  if (!admin.status) throw new Error("Account is inactive, contact super admin");

  const isMatch = await bcrypt.compare(password, admin.password);
  if (!isMatch) throw new Error("Invalid email or password");

  const roles = admin.roles.map((r) => r.code);
  const permissions = admin.roles.flatMap((r) => r.permissions.map((p) => p.name));

  const accessToken = generateToken({ id: admin.id, email: admin.email, roles, permissions });

  admin.remember_token = accessToken;
  await saveAdmin(admin);

  await logActivity({ created_by: admin.id, action: "LOGIN", module: "Admin" });
